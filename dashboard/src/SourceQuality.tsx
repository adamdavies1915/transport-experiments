...
export default function SourceQuality({ data: supplied }: { data?: SourceQualityData } = {}) {
  ...
  return <aside aria-label="Observation source coverage" className="rounded-xl border border-slate-700/70 px-4 py-4 sm:px-5 text-xs text-slate-300">
    {snapshot?.stale && <p className="mb-3 text-amber-200" role="status">{!snapshot.generated_at ? 'The first analysis is still being prepared.' : snapshot.refresh_error ? 'Summary refresh is unavailable; showing the saved analysis.' : 'The saved analysis is older than expected; the next result is pending.'} Collection may continue while analysis is behind. The figures below describe the saved period.</p>}
